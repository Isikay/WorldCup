...
18. **Gerçekçi Penaltı Atışları Simülasyonu (Penalty Shootout Algorithm):** Eleme turlarında (Çeyrek Final, Yarı Final, Final) beraberlik durumunda artık rastgele sonuç yerine tam bir penaltı atışları simülasyonu çalışır. Her takımdan şut istatistiği en yüksek 5 saha oyuncusu otomatik seçilir. Her atışın gol olma olasılığı, atıcının şut (sho) istatistiği, kalecinin kurtarış yeteneği (def) ve oyuncunun stamina seviyesine göre hesaplanır. Sonuçlar ✅ (gol), 🧤 (kaleci kurtarışı), ❌ (kaçırma/direk) olarak izlenir. 5 tur sonunda eşitlik devam ederse sudden death (seri penaltı) uygulanır. Penaltı sonuçları maç sonu ekranında `2 - 2 (pen: 4-3)` formatında ve bracket tablosunda `2 (4p)` formatında gösterilir. Arka plandaki CPU vs CPU maçları da aynı algoritmayı kullanır.
19. **Maç Başına 5 Değişiklik Limiti (5 Substitution Limit):** Gerçek futbol kurallarına uygun olarak maç başına maksimum oyuncu değişikliği sayısı 8'den 5'e düşürüldü. Yedek kulübesi boyutu 8 oyuncu olarak korunarak, 8 yedekten en fazla 5'inin maç boyunca sahaya alınabilmesi sağlandı. Bu limit hem kullanıcı hem de CPU takımları için geçerlidir.
